...
// Wir hängen einen Datensatz an oder geben einen euen ein: 
// n=name, v=vorname; at=alter.tage, am=alter.monat, aj=alter.jahr
// eint=eingestellt tag, einm=eingestellt monat, einj=eingest.
// Jahr g=gehalt

void anhaengen(char *n, char *v, int at, int am, int aj, int eint,
		int einm, int einj, long g){
	// zeiger zum Zugriff auf die einzelnen Elemente der Struktur
	struct angestellt *zeiger;

	// Wir fragen ab, ob es chon ein Element in der Liste gibt. Wir suchen
	// das Element auf das unser Zeiger *anfang zeigt. Falls *anfang imme rnoch
	// auf NULL zeigt, bekommt *anfang die Adresse unseres 1. Elements ud ist 
	// somit der Kopf (Anfang) unserer List.
	if(anfang == NULL){
		// Wir reservieren Speicherplatz für unsere Struktur für das
		...
		// Somit haben wir unseren Anfang der Liste, Von nun an zeigt der Zeiger
		// anfang immer auf das Element vor ihm. Da dies aber jetzt das 1 . Element
		// der Liste war, zeigt der Zeier anfang auf den Zeiger next, next zeigt
		// am Ende immer wieder NULL.
		anfang->next = NULL;

	}

	// Es scheint scho mindesteins ein Element in der Liste vorhanden zu sein, da der
	// Anfang nicht == NULL ist. 
	// Jetzt suchen wir so lange nach dem nächsten Element bis der *next-Zeiger auf 
	// NULL zeigt. Somit haben wir das Ende der Liste grunden und können einen 
	// neuen Datensatz anhägen
	else{
		zeiger=anfang; // Wir zeigen auf das 1. Element
		while(zeiger->next != NULL)
			zeiger = zeiger->next;

		// Wir reservieren einen Speicherplatz für das letzt Element
